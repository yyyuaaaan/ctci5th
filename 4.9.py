...
方案1：如果结点中包含指向父亲结点的指针，那么，只需要去遍历这棵二叉树， 然后从每个结点开始，不断地去累加上它父亲结点的值直到父亲结点为空
...
看起来简单，但是没有考虑两个子节点之间的节点。

方案2：如果结点中不包含指向父亲结点的指针，则在二叉树从上向下查找路径的过程中， 需要为每一次的路径保存中间结果，累加求和仍然是从下至上的，
对应到保存路径的数组， 即是从数组的后面开始累加的，这样能保证遍历到每一条路径。
...
"""
当然， preorder traversal， O(N)time, every node, log(n) deep
timecomplexity
Well,if a node is at level r, we do r amount of work (that's in the looking "up" step).
We can take aguessat0(n log(n)) sincetherearennodesdoinganaverageoflog(n) amount of work on each step.

Thespacecomplexityis0(log n),sincethealgorithmwillrecurse0(log n)times and the path parameter
is only allocated once (atO(log n) space)duringthisrecursion.
"""