...
def convert(root):
    """
    left uses as predecessor, right used as successor, just recursively add more links
    """
    if root is None: return None
    ...
    def concatena(x,y):  # concatenate, better define a new function
        x.right = y
        y.left = x

    if part1 is not None:  concatena(part1[1],root)
    if part2 is not None:  concatena(root, part2[0])
